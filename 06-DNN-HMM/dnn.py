...
class ReLU(Layer):
    ...
    def backward(self, input, output, d_output):
        # BEGIN_LAB
        return np.where(input > 0, d_output, 0)
        # END_LAB
...
class FullyConnect(Layer):
    ...
    def forward(self, input):
        # BEGIN_LAB
        return np.matmul(input, self.w.T) + self.b
        # END_LAB
    ...
    def backward(self, input, output, d_output):
        ...
        # input:[dim(batch_size), dim(lay)],
        # output:[dim(batch_size), dim(lay+1)],
        # d_output:[dim(batch_size), dim(lay+1)]
        self.dw = np.dot(d_output.T, input)  # [dim(lay+1), dim(lay)]
        self.db = np.sum(d_output, axis=0, keepdims=True)  # [1, dim(lay+1)]
...
def train(dnn):
    ...
    for i in range(num_epochs):
        ...
        while cur < num_samples:
            ...
            input = inputs[cur:end]  # num_samples*num_feats
...
def main():
    ...
    dnn = DNN(429, 11, 200, 1)  # (429, 11, 128, 1)
